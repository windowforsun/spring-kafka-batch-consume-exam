...
import com.windowforsun.kafka.batch.consume.consumer.SpringKafkaBatchConsumer;
import com.windowforsun.kafka.batch.consume.util.DemoConsumerInterceptor;

import lombok.extern.slf4j.Slf4j;

@Slf4j
@SpringBootTest(classes = SpringKafkaBatchConfig.class)
@DirtiesContext(classMode = DirtiesContext.ClassMode.AFTER_CLASS)
@ActiveProfiles("test-spring-kafka-batch")
@Import(TestConfig.class)
@EmbeddedKafka(controlledShutdown = true, topics = {"inbound-topic-1", "outbound-topic"}, partitions = 1)
public class SpringKafkaBatchConsumerTest {
	@Autowired
	...
	public void success_for_consume() throws ExecutionException, InterruptedException {
		SpringKafkaBatchConsumer.shouldError = false;
	...
	public void consume_fail_on_testKey() throws Exception {
		...
		Awaitility.await().atMost(5, TimeUnit.SECONDS)
			.pollDelay(100, TimeUnit.MILLISECONDS)
			.until(TestConfig.KafkaTestListener.counter::get, is(5));
		...
		assertThat(DemoConsumerInterceptor.POLL_ITEMS.get(1), containsInAnyOrder("inbound-topic-1::1",
			...
			"inbound-topic-1::3"));
		assertThat(DemoConsumerInterceptor.POLL_ITEMS.get(2), containsInAnyOrder("inbound-topic-1::4"));
		assertThat(outboundReceivedKeyList, hasSize(5));
		assertThat(outboundReceivedKeyList.get(0), is("1"));
		assertThat(outboundReceivedKeyList.get(1), is("1"));
		assertThat(outboundReceivedKeyList.get(2), is("2-test"));
		assertThat(outboundReceivedKeyList.get(3), is("3"));
		assertThat(outboundReceivedKeyList.get(4), is("4"));
	}
}
